...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Auto-advance (mirror phase) sends no user message — only block in shadow phase
    if (!latestInput && session.phase !== "mirror") {
      return res.status(400).json({ error: "No user message" });
    }

    // Add user turn to history (only when there is a message — not on auto-advance)
    if (latestInput) {
